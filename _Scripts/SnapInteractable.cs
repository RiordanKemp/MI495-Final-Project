...
public class SnapInteractable : MonoBehaviour
{
    ...
    void Update(){
        if (Input.GetKeyDown(settingsManager.controls["interact"])){
            player.interactBufferRemaining = interactBufferTime;
            player.interactBufferActive = true;
        }
      

        if (!cooldownActive){
            if (active && player.interactBufferActive){
