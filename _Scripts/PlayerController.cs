...
[Header("Inscribed")]
 ...
 [Tooltip("The amount of time the player has to input a leap after leaving a climbable wall.")]
    ...
    [Header("Dynamic")]
       public float interactBufferRemaining = 0;
    public bool interactBufferActive = false;
    ...
    void Update(){
        ...
        if (interactBufferRemaining > 0){
            interactBufferRemaining -= Time.deltaTime;
        }
        else if (interactBufferActive && interactBufferRemaining <= 0){
            ResetBuffer();
        }
    }

           public void ResetBuffer(){
            interactBufferActive = false;
            interactBufferRemaining = 0;
        }
